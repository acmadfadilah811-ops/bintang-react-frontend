...
import {
  ...
  X, Save, TrendingUp, Loader2, BarChart2, RefreshCw, History, ChevronDown, ChevronUp, Download
} from 'lucide-react';

const KATEGORI_LIST = ['Tinta', 'Bahan Cetak', 'Finishing', 'Alat', 'Packaging', 'Lainnya'];

const DEFAULT_FORM = {
  nama: '', stok: '', satuan: '', kategori: 'Tinta',
  min_stok: '', cost_per_unit: '', supplier: ''
};
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          {isManager && (
            <button onClick={handleExport} className="flex items-center gap-1.5 bg-green-600 hover:bg-green-700 text-white text-sm font-bold px-4 py-2 rounded-lg shadow transition-all">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <button onClick={fetchItems} className="p-2 text-slate-400 hover:text-indigo-600 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
          <RefreshCw size={15} />
        </button>
      </div>
                    ...
                    <>
                      <tr
                        ...
                        className={`hover:bg-slate-50 transition-colors ${
                          isIdInvalid ? 'bg-orange-50/60 border-l-4 border-l-orange-400' :
                          isKritis ? 'bg-red-50/60' : ''
                        }`}
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2 flex-wrap">
                            {isKritis && <AlertTriangle size={13} className="text-red-500 shrink-0" />}
                            {isIdInvalid && (
                            ...
                            <span className={`font-semibold ${
                              isIdInvalid ? 'text-orange-700' :
                              isKritis ? 'text-red-700' : 'text-slate-900'
                            }`}>
                              {item.nama}
                            </span>
                          </div>
                        ...
                        <td className="px-4 py-3 text-center">
                          <div className="flex flex-col items-center gap-1">
                            <span className={`font-bold text-base ${isKritis ? 'text-red-600' : 'text-slate-800'}`}>
                              {item.stok}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-1">
                            {isIdInvalid ? (
                              ...
                              <>
                                <button
                                  onClick={() => { setAdjustItem(item); setAdjustDelta(''); setAdjustNote(''); }}
                                ...
                                <button
                                  onClick={() => setHistoryItem(historyItem?.id === item.id ? null : item)}
                                  className={`p-1.5 rounded transition-colors ${
                                    historyItem?.id === item.id ? 'text-indigo-700 bg-indigo-100' : 'text-slate-500 hover:bg-slate-100'
                                  }`}
                                  title="Riwayat"
                                >
                                  {historyItem?.id === item.id ? <ChevronUp size={14}/> : <History size={14}/>}
                                </button>
                                {isManager && (
                                  <>
                                    <button onClick={() => setEditItem(item)} className="p-1.5 text-indigo-600 hover:bg-indigo-50 rounded transition-colors" title="Edit">
                                      <Edit2 size={14} />
                                    </button>
                                    <button onClick={() => setDeleteItem(item)} className="p-1.5 text-red-500 hover:bg-red-50 rounded transition-colors" title="Hapus">
                                      <Trash2 size={14} />
                                    </button>
                                  </>
                                )}
                            ...
                            <div className="text-xs space-y-2">
                              ...
                              {(!item.history || item.history.length === 0) ? (
                                ...
                                <div className="space-y-1">
                                  {item.history.map(h => (
                                    <div key={h.id} className="flex gap-4 p-2 bg-white rounded border border-slate-200">
                                      <span className="font-mono text-slate-400 w-24">{new Date(h.waktu).toLocaleDateString()}</span>
                                      <span className={`font-bold w-16 ${h.delta > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                                        {h.delta > 0 ? '+' : ''}{h.delta}
                                      </span>
                                      <span className="text-slate-600 flex-1">{h.keterangan || '-'}</span>
                                      <span className="text-slate-400 text-[10px] italic">Oleh: {h.user_nama}</span>
                                    </div>
                                  ))}
                                </div>
                              )}
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="bg-emerald-700 text-white px-5 py-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-emerald-200 text-xs mt-0.5">{adjustItem.nama} — Stok: <strong>{adjustItem.stok} {adjustItem.satuan}</strong></p>
              </div>
              <button onClick={() => setAdjustItem(null)}><X size={18} /></button>
            </div>
            <form onSubmit={handleAdjust} className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">
                  Perubahan Stok <span className="text-slate-400 font-normal">(+ tambah, − kurangi)</span>
                </label>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setAdjustDelta(v => String((parseFloat(v)||0) - 1))}
                    className="px-3 py-2 bg-red-100 text-red-700 font-bold rounded-lg hover:bg-red-200 transition-colors">−</button>
                  <input
                    type="number" step="0.1" value={adjustDelta}
                  ...
                  />
                  <button type="button" onClick={() => setAdjustDelta(v => String((parseFloat(v)||0) + 1))}
                    className="px-3 py-2 bg-emerald-100 text-emerald-700 font-bold rounded-lg hover:bg-emerald-200 transition-colors">+</button>
                </div>
                {adjustDelta && (
                  <p className="text-xs text-slate-500 mt-1 text-center">
                    Stok baru: <strong className="text-slate-800">
                      {Math.max(0, adjustItem.stok + (parseFloat(adjustDelta)||0))} {adjustItem.satuan}
                    </strong>
                  </p>
                )}
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={() => setAdjustItem(null)} className="flex-1 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">Batal</button>
                <button type="submit" disabled={saving || !adjustDelta}
                  ...
                  {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Simpan Restock
                </button>
              </div>
            </form>
          </div>
      ...
      {deleteItem && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-center space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setDeleteItem(null)} className="flex-1 py-2 text-sm border border-slate-200 rounded-lg hover:bg-slate-50">Batal</button>
              <button onClick={handleDelete} disabled={saving} className="flex-1 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-500 rounded-lg disabled:opacity-50">
          ...
          <div className="grid grid-cols-2 gap-3">
            {showIdField && (
              <div className="col-span-2">
                ...
                <input name="id" defaultValue={defaultValues.id} required placeholder="contoh: INV-TINTA-01"
            ...
            <div className="col-span-2">
              ...
              <input name="nama" defaultValue={defaultValues.nama} required placeholder="Tinta Sublim Hitam"
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Stok Saat Ini</label>
              <input name="stok" type="number" step="0.1" defaultValue={defaultValues.stok} required
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Satuan *</label>
              <input name="satuan" defaultValue={defaultValues.satuan} required placeholder="liter, kg, pcs, rol..."
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Min. Stok (Alert)</label>
              <input name="min_stok" type="number" step="0.1" defaultValue={defaultValues.min_stok}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
            ...
            <div>
              ...
              <input name="cost_per_unit" type="number" defaultValue={defaultValues.cost_per_unit}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Supplier</label>
              <input name="supplier" defaultValue={defaultValues.supplier} placeholder="Nama supplier/toko"
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
          </div>
          <div className="flex gap-2 pt-2 border-t border-slate-100">
            <button type="button" onClick={onClose} className="flex-1 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">Batal</button>
            <button type="submit" disabled={saving} className="flex-1 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg disabled:opacity-50 flex items-center justify-center gap-2">
